...
import { useContribute } from '@/features/wallet/hooks/useWallet';
...
export function BookingCard({ booking, walletBalance }: Props) {
    ...
    const handleContribute = () => {
        if (walletBalance < booking.totalAmount) {
            toast.error(`Insufficient balance! You need ₹${(booking.totalAmount - walletBalance).toFixed(2)} more.`);
            return;
        }
    ...
    return (
        <div className="flex flex-col gap-4 border-b p-5 last:border-0 hover:bg-gray-50/50 transition-all rounded-xl mt-2 font-sans">
            <div className="flex items-start justify-between gap-4">
                ...
                <div className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${statusColor[booking.status]}`}>
                    {booking.status}
                </div>
            </div>
            ...
            <div className="flex flex-wrap gap-1.5 grayscale opacity-70">
                {booking.items.slice(0, 3).map((item) => (
                    <span key={item.id} className="text-[10px] px-2 py-0.5 bg-gray-100 rounded-md text-gray-600 border">
            ...
            {booking.status === 'PENDING' && (
                <div className="space-y-3 mt-1">
                    <div className="flex items-center justify-between text-[11px] font-bold">
                        ...
                        <span className={`${walletBalance < booking.totalAmount ? 'text-red-500' : 'text-green-600'} flex items-center gap-1`}>
                    ...
                    <button
                        ...
                        disabled={isPending}
                        className={`flex items-center justify-center gap-2 w-full p-3 rounded-xl font-bold uppercase tracking-tight transition-all active:scale-[0.98] shadow-md ${walletBalance < booking.totalAmount
                    ...
                    {walletBalance < booking.totalAmount && (
                        <p className="text-[10px] text-center text-red-400 font-medium">Please top up your wallet to pay for this booking.</p>
                    )}
                </div>
            )}
        </div>
