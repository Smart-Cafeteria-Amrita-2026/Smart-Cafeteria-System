        ...
        <div className="mx-auto max-w-xl space-y-6 p-4 pb-20">
            ...
            <div className="rounded-2xl border bg-white p-6 shadow-sm space-y-4">
                ...
                <div className="grid gap-3">
                    <button
                        className={`flex w-full items-center justify-between rounded-xl border-2 p-4 font-bold transition-all active:scale-95 ${paymentMode === 'PAY_LATER'
                    ...
                    <button
                        className={`flex w-full items-center justify-between rounded-xl border-2 p-4 font-bold transition-all active:scale-95 ${paymentMode === 'GROUP_SPLIT'
                                ? 'border-blue-600 bg-blue-50 text-blue-600'
                                : 'border-gray-100 bg-white text-gray-500'
                            }`}
                ...
                {paymentMode === 'GROUP_SPLIT' && (
                    <div className="space-y-3 border-t pt-4 animate-in slide-in-from-top-2 duration-300">
                        <label className="text-sm font-bold text-gray-700">Add Registered Members</label>
                        ...
                        {memberRolls.length > 0 && (
                            <div className="flex flex-wrap gap-2 pt-2">
                                {memberRolls.map((roll) => (
                                    ...
                                    >
                                        <span>{roll}</span>
                                        <button onClick={() => removeMember(roll)} className="hover:text-red-500">×</button>
                                    </div>
                                ))}
                            </div>
                        )}
                        <p className="text-[10px] text-gray-400">Members will receive a payment request on their dashboard.</p>
                    </div>
                )}
            </div>
